...
* **Stage 2: Voltage Selector (`VT_Processor_Py_Stage2_VoltageSelector_v1.7.0.py`)**
    ...
    * **Action:**
        1.  Reads `PhaseWidth(µs)` and `InterphaseDelay(µs)` from the metadata.
    ...
    * **Output:** The final summary table (`PyVS_VoltageSummary_Final.CSV`), which merges your original metadata with all the new calculated voltage values (V1-V5, ΔV, etc.).
...
* **Columns:** The script does not rely on fixed column positions. Instead, it auto-detects the necessary columns by searching for common names (aliases).
    * **Time:** Searches for `Time(s)`, `Time (µs)`, `T (ms)`, etc., and converts to microseconds (µs).
...
* **Stage 2 Calculation Columns (Required):**
    * **`PhaseWidth(µs)`:** User-defined. This value is **read by Stage 2** to auto-calculate the initial `t3` (Vc peak) and `t4` (Emc) time points.
    * **`InterphaseDelay(µs)`:** User-defined. This value is **read by Stage 2** to auto-calculate the initial `t5` (E IPend) time point.
* **User-Defined Info Columns (Recommended):**
    ...
    * Examples: `Date`, `WaferID`, `DeviceID`, `ElectrodeID`, `Electrolyte`, `Current(µA)`, etc.
...
* **`PyVS_VoltageSummary_Final.CSV`**
    ...
    * **Contents:** This is the main result. It is a single CSV file containing all columns from your original `Metadata.xlsx` *plus* all the new calculated voltage columns (e.g., `Eipp (V1)`, `Vc peak (V3)`, `ΔV (V3-V1)`, etc.) for every processed file.
...
├── VT_Module_Py_Paths.py
├── VT_Module_Py_Utilities.py
├── VT_Module_Py_PulseAlgorithm.py
├── VT_Module_Py_PlotHelpers.py
│
...
└── 📁 Example_Data_Files/
    ...
    ├── 📄 TEK00003.CSV
    ├── 📄 TEK00004.CSV
    ├── 📄 TEK00005.CSV  ... TEK000xx.CSV
    └── 📁 VT_Py_Outputs/
        ├── 📁 VT_Py_Outputs_PulseDetector_YYYYMMDD_HHMM/
        ├── 📁 VT_Py_Outputs_VoltageSelector_YYYYMMDD_HHMM/
        └── 📁 VT_Py_Outputs_BatchPlotter_YYYYMMDD_HHMM/ 
    └── 📁 VT_Py_Outputs/
        ├── 📁 VT_Py_Outputs_PulseDetector_YYYYMMDD_HHMM
        │   ├── 📄 PyPD_Summary.txt
        │   ├── 📄 PyPD_Summary.json
        │   ├── 📄 PyPD_Dictionary_AllDataFiles_wAutoOffset.pkl
        │   │
        │   └── 📁 PyPD_ProcessedDataFiles/
        │       ├── 📁 PyPD_CSV_Files/
        │       │   ├── 📄 PyPD_TEK0001_AutoOffset.csv
        │       │   ├── 📄 PyPD_TEK0002_AutoOffset.csv
        │       │   └── ...
        │       │
        │       ├── 📁 PyPD_PNG_Files/
        │       │   ├── 🖼️ PyPD_TEK0001_AutoOffset.png
        │       │   ├── 🖼️ PyPD_TEK0002_AutoOffset.png
        │       │   └── ...
        │       │
        │       └── 📁 PyPD_ExtraInfo_Files/
        │           ├── 📄 PyPD_Dictionary_AllDataFiles_wAutoDetected.pkl
        │           ├── 📄 PyPD_Dictionary_AllDataFiles_wRawTimeµs.pkl
        │           ├── 📄 PyPD_TEK0001_RawTimeµs.csv
        │           ├── 📄 PyPD_TEK0001_Detected.csv
        │           └── ...
        │
        ├── 📁 VT_Py_Outputs_VoltageSelector_YYYYMMDD_HHMM
        │   ├── 📄 PyVS_VoltageSummary_Final.CSV
        │   ├── 📄 PyVS_Dictionary_AllDataFiles_SelectedVals.pkl
        │   ├── 📄 PyVS_Summary.txt
        │   ├── 📄 PyVS_Summary.json
        │   │
        │   └── 📁 PyVS_ProcessedDataFiles/
        │       ├── 📁 PyVS_CSV_Files/
        │       │   ├── 📄 PyVS_TEK0001_SelectedVals.csv
        │       │   ├── 📄 PyVS_Metadata_Input.CSV
        │       │   ├── 📄 PyVS_VoltageSummary_CalculatedVals.CSV
        │       │   └── ...
        │       │
        │       └── 📁 PyVS_PNG_Files/
        │           ├── 🖼️ PyVS_TEK0001_SelectedVals_wOverlay.png
        │           ├── 🖼️ PyVS_TEK0002_SelectedVals_wOverlay.png
        │           └── ...
        │
        └── 📁 VT_Py_Outputs_BatchPlotter_YYYYMMDD_HHMM
            ├── 📄 PyBP_Summary.txt
            ├── 📄 PyBP_Summary.json
            ├── 📄 PyBP_Dictionary_AllDataFiles_Processed.pkl
            │
            └── 📁 PyBP_ProcessedDataFiles/
                ├── 📁 PyBP_CSV_Files/
                │   ├── 📄 PyBP_TEK00001_UserOffset.csv
                │   ├── 📄 PyBP_TEK00002_Raw.csv
                │   └── ...
                │
                ├── 📁 PyBP_PNG_Files/
                │   ├── 🖼️ PyBP_Current_wBatchPlot_{custom_suffix}.png
                │   ├── 🖼️ PyBP_Voltage_wBatchPlot_{custom_suffix}.png
                │   └── ...
                │
                └── 📁 PyBP_SVG_Files/
